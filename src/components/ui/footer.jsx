...
const Footer = ({ portfolioData }) => {
  ...
  return (
    <footer className="relative mt-32 bg-card">
      {/* Decorative top border */}
      <div className="absolute top-0 left-0 right-0 h-px bg-gradient-to-r from-transparent via-[#821d30]/30 to-transparent"></div>
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        {[...Array(20)].map((_, i) => (
          <motion.div
            key={i}
            className="absolute w-1 h-1 bg-[#821d30]/20 rounded-full"
            style={{
              left: `${Math.random() * 100}%`,
              top: `${Math.random() * 100}%`,
            }}
      ...
      <div className="relative max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        ...
        <div className="text-center space-y-8">
          ...
          <motion.div
            ...
            className="space-y-4"
          >
            <h3 className="text-2xl font-bold text-[#821d30]">
              {portfolioData?.name}
              <span className="text-[#821d30]">.dev</span>
            </h3>
            <p className="text-[#2c2a2b] max-w-2xl mx-auto">
              Building digital experiences that matter. Always learning, always growing, always coding.
            </p>
          </motion.div>
          ...
          >
            <a
              href="#about"
              className="text-[#2c2a2b] hover:text-[#821d30] transition-colors duration-200"
            >
              About
            </a>
            <a
            ...
            <Link
              to="/blogs"
              className="text-[#821d30] hover:text-[#2c2a2b] transition-colors duration-200 font-medium"
            >
              My Blogs
            </Link>
          </motion.div>
          ...
          >
            {socialLinks.map(({ key, icon: Icon, url, label }) => url && (
              <motion.a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="group relative p-3 rounded-full border border-[#2c2a2b] hover:border-[#821d30]/50 bg-[#2c2a2b]/30 hover:bg-[#821d30]/10 transition-all duration-300"
                whileHover={{ scale: 1.1 }}
                whileTap={{ scale: 0.95 }}
                aria-label={label}
              >
                <Icon className="w-5 h-5 text-[#2c2a2b] group-hover:text-[#821d30] transition-colors duration-300" />

                {/* Tooltip */}
                <div className="absolute -top-12 left-1/2 transform -translate-x-1/2 opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none">
                  <div className="bg-[#2c2a2b] text-[#dcd7d4] text-xs px-2 py-1 rounded-md border border-[#2c2a2b]">
                    {label}
                  </div>
                  <div className="w-2 h-2 bg-[#2c2a2b] border-r border-b border-[#2c2a2b] transform rotate-45 absolute top-full left-1/2 -translate-x-1/2 -translate-y-1/2"></div>
                </div>
              </motion.a>
            ))}
          </motion.div>
          ...
          {portfolioData?.contact?.location && (
            <motion.div
              ...
              transition={{ duration: 0.6, delay: 0.3 }}
              viewport={{ once: true }}
              className="flex items-center justify-center gap-2 text-[#2c2a2b]"
            >
              <MapPin className="w-4 h-4 text-[#821d30]" />
              <span className="text-sm">{portfolioData.contact.location}</span>
            </motion.div>
          )}
          ...
          <motion.div
            ...
            transition={{ duration: 0.8, delay: 0.4 }}
            viewport={{ once: true }}
            className="h-px bg-gradient-to-r from-transparent via-[#2c2a2b] to-transparent mx-auto"
          ></motion.div>
          ...
          >
            {/* Fun stats or quote */}
            <div className="flex flex-wrap justify-center gap-6 text-sm text-[#2c2a2b]">
              <div className="flex items-center gap-1">
                <Code className="w-4 h-4 text-[#821d30]" />
                <span>Clean Code Enthusiast</span>
              </div>
              <div className="flex items-center gap-1">
                <Coffee className="w-4 h-4 text-[#821d30]" />
                <span>Powered by Coffee</span>
              </div>
              <div className="flex items-center gap-1">
                <Sparkles className="w-4 h-4 text-[#821d30]" />
                <span>Innovation Driven</span>
              </div>
            </div>

            {/* Copyright */}
            <div className="flex items-center justify-center gap-2 text-sm text-[#2c2a2b]">
              <span>&copy; {currentYear} {portfolioData?.name}.</span>
              <span>Made with</span>
              <Heart className="w-4 h-4 text-[#821d30] animate-pulse" />
              <span>and lots of</span>
              <Coffee className="w-4 h-4 text-[#c3b79d]" />
            </div>

            {/* Tech stack */}
            <p className="text-xs text-[#2c2a2b]">
              Built with React, Tailwind CSS & Framer Motion
            </p>
          </motion.div>
        </div>
        ...
        {/* Background glow effects */}
        <div className="absolute inset-0 -z-10">
          <div className="absolute bottom-0 left-1/4 w-64 h-32 bg-[#821d30]/5 rounded-full blur-3xl"></div>
          <div className="absolute bottom-0 right-1/4 w-64 h-32 bg-[#c3b79d]/5 rounded-full blur-3xl"></div>
        </div>
      </div>

      {/* Bottom gradient */}
      <div className="absolute bottom-0 left-0 right-0 h-24 bg-gradient-to-t from-[#dcd7d4] to-transparent"></div>
    </footer>
  );
};
